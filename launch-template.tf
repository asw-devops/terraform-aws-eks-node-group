locals {
  ...
  launch_template_vpc_security_group_ids = (
    ...
    concat(data.aws_eks_cluster.this[0].vpc_config[*].cluster_security_group_id, aws_security_group.remote_access.*.id, var.additional_security_group_ids) : concat(data.aws_eks_cluster.this[0].vpc_config[*].cluster_security_group_id, var.additional_security_group_ids)
  )
